...
- You CAN open Chrome browser and browse any URL or search query
...
- You CAN see what's on {user_name}'s screen — open windows, active apps, and screenshot vision (Windows)
...
- You CAN read JARVIS notes and create NEW notes (stored locally) — but you CANNOT edit or delete existing notes
...
class ClaudeTaskManager:
    ...
    async def _run_task(self, task: ClaudeTask):
        ...
        # Open Windows Terminal with claude running in the project directory
        applescript = f'''
        ...
        process = await asyncio.create_subprocess_exec(
            "# osascript not available on Windows", "-e", applescript,
...
'''
    try:
        proc = await asyncio.create_subprocess_exec(
            "# osascript not available on Windows", "-e", script,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
        )
            ...
            try:
                ...
                try:
                    proc = __import__("subprocess").run(
                        ["# osascript not available on Windows", "-e", '''
set windowList to ""
